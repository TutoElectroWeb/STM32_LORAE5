...
/**
  ******************************************************************************
  * @file           : test_lorae5_mode_rx.c
  * @brief          : Exemple mode TEST RX continu LoRa-E5
  ******************************************************************************
  */
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define LOG_NAME "test_lorae5_mode_rx"          ///< Nom du journal pour affichage bannière
/* USER CODE END PD */
...
/* USER CODE BEGIN PFP */
static void LoraTestRx_AppInit(void);
static void LoraTestRx_AppTask(void);
/* USER CODE END PFP */
...
static void LoraTestRx_AppTask(void)
{
  ...
  switch (test_step)
  {
    ...
    default:
      if (LORAE5_SendAT(&hlorae5, "AT+TEST=RXLRPKT") == LORAE5_OK)
      {
        printf("[APP] RXLRPKT actif, attente paquets...\r\n");
        next_action_ms = now + 15000U;
      }
      break;
  }
}
